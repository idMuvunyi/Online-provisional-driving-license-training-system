...
if (isset($_SESSION['key'])) {
    if (@$_GET['deidquiz'] && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        while($row = mysqli_fetch_array($q)){
            $user = $row['phone'];
            $s = $row['score'];
            $r1 = mysqli_query($con, "UPDATE history SET status='finished',score_updated='true' WHERE eid='$eid' AND phone='$user' ") or die('Error');
            $q1 = mysqli_query($con, "SELECT * FROM rank WHERE phone='$user'") or die('Error161');
            ...
            if ($rowcount == 0) {
                ...
                $sun = $s + $sun;
                $q3 = mysqli_query($con, "UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE phone= '$phone'") or die('Error174');
            }
        }
...
if (isset($_SESSION['key'])) {
    if (@$_GET['dusername'] && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        $r1 = mysqli_query($con, "DELETE FROM rank WHERE phone='$dusername' ") or die('Error');
        $r2 = mysqli_query($con, "DELETE FROM history WHERE phone='$dusername' ") or die('Error');
        $result = mysqli_query($con, "DELETE FROM user WHERE phone='$dusername' ") or die('Error');
...
if (isset($_SESSION['key'])) {
    if (@$_GET['id'] && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $dusername = @$_GET['id'];
        $r1 = mysqli_query($con, "DELETE FROM rank WHERE id='$dusername' ") or die('Error');
...
if (isset($_SESSION['key'])) {
    ...
    if (@$_GET['delet'] && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $duser = @$_GET['delet'];
        $id = @$_GET['id'];
        $r1 = mysqli_query($con, "DELETE FROM questions WHERE id='$duser' ") or die('Error');
        header("location:b.php?id='$id'");
    }
}
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquizs' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $num = $_SESSION['eds'];
        $total   = $_POST['total'];
        $id      = uniqid();
        header("location:b.php?id=$num&step=2&n=$total");
    }
}
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $q3   = mysqli_query($con, "INSERT INTO questions VALUES  (NULL,'$eid','$qid','$qns','$image', '$ch' , '$i')") or die();
            ...
            $qd = mysqli_query($con, "INSERT INTO options VALUES  (NULL,'$qid','$e','$oeid')") or die('Error65'.mysqli_error($con));
            $f = $_POST['ans' . $i];
            switch ($f) {
                case 'a':
                    $ansid = $oaid;
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addqnss' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $q3   = mysqli_query($con, "INSERT INTO questions VALUES  (NULL,'$eid','$qid','$qns','$image', '$ch' , '$total')") or die();
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_POST['ans']) && (!isset($_GET['delanswer']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            $q = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND phone='$_SESSION[phone]' AND qid='$qid' ") or die('Error115');
            while ($row = mysqli_fetch_array($q)) {
                $prevans = $row['ans'];
            }
            ...
            $q = mysqli_query($con, "SELECT * FROM user_answer WHERE phone='$_SESSION[phone]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error1977');
            if (mysqli_num_rows($q) != 0) {
                $q = mysqli_query($con, "UPDATE user_answer SET ans='$ans' WHERE phone='$_SESSION[phone]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error197');
            } else {
                $q = mysqli_query($con, "INSERT INTO user_answer VALUES(NULL,'$qid','$ans','$ansid','$_GET[eid]','$_SESSION[phone]')");
            }
            ...
            if ($ans == $ansid) {
                ...
                $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$eid' AND phone='$phone' ") or die('Error115');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $r = $row['correct'];
                    $w = $row['wrong'];
                ...
                if (isset($prevans) && $prevans == $ansid) {
                    ...
                    $w--;
                    $s = $s + $correct + $wrong;
                    $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`correct`=$r,`wrong`=$w, date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error13');
                } else {
                    $r++;
                    $s = $s + $correct;
                    $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`correct`=$r, date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error14');
                }
            } else {
                ...
                $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$eid' AND phone='$phone' ") or die('Error139');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $w = $row['wrong'];
                    $r = $row['correct'];
                ...
                if (isset($prevans) && $prevans != $ansid) {
                    ...
                    $w++;
                    $s = $s - $correct - $wrong;
                    $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w,`correct`=$r, date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error11');
                } else {
                    $w++;
                    $s = $s - $wrong;
                    $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w,date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error12');
                }
            }
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && (!isset($_POST['ans'])) && (isset($_GET['delanswer'])) && $_GET['delanswer'] == "delanswer") {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            $ans = $row['ans'];
            $q = mysqli_query($con, "DELETE FROM user_answer WHERE qid='$qid' AND phone='$_SESSION[phone]' AND eid='$_GET[eid]' ") or die("Error2222");
            if ($ans == $ansid) {
                $q = mysqli_query($con, "SELECT * FROM quiz WHERE eid='$eid' ") or die('Error129');
                while ($row = mysqli_fetch_array($q)) {
                    $wrong   = $row['wrong'];
                ...
                $r--;
                $s = $s - $correct;
                $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`correct`=$r, date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error11');
            } else {
                $q = mysqli_query($con, "SELECT * FROM quiz WHERE eid='$eid' ") or die('Error129');
                while ($row = mysqli_fetch_array($q)) {
                    $wrong   = $row['wrong'];
                ...
                $w--;
                $s = $s + $wrong;
                $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w, date= NOW()  WHERE  phone = '$phone' AND eid = '$eid'") or die('Error11');
            }
